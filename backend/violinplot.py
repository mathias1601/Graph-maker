...
def violin_plot(groupName: str, columnValue: str, csvFile: File, title: str = "Title"):
    ...
    # To replace NaN-values with 0 from lists since violinplot-function cannot handle them
    cleaned_values = [[0 if np.isnan(value) else value for value in group] for group in values]
    ...
    # Generate plots
    fig, ax = plt.subplots()
    ax.violinplot(cleaned_values)
    ax.set_title(title)
    ax.set_xticks([x + 1 for x in range(len(cleaned_values))], labels=labels)
